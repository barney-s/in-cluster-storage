...
COMPUTE_SVC_ACCT=$(gcloud iam service-accounts list --project="${PROJECT}" --filter="email:compute@developer.gserviceaccount.com" --format="value(email)" | head -n1)
if [ -z "${COMPUTE_SVC_ACCT}" ]; then
...
if [ -n "${COMPUTE_SVC_ACCT}" ]; then
  echo "Granting artifactregistry.reader role to service account: ${COMPUTE_SVC_ACCT}"
